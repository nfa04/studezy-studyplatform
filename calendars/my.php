<?php
    ...
    // This file is almost identical to view.php but automatically assumes the calendar to be loaded to be the default calendar for this user
...
<html>
    <head>
        <title>My calendar | StudEzy</title>
    ...
    <body>
        ...
        <div class="calendar_mode_selectors">
            ...
            ?><a href="?m=<?php echo $_GET['m']; ?>&t=<?php $t = explode('-', (isset($_GET['t']) ? $_GET['t'] : date($dformat)));  $t[count($t) - 1]--; echo implode('-', $t); ?>"><img src="/res/img/arrow_back.svg"></a>
            <a href="?m=<?php echo $_GET['m']; ?>&t=<?php $t = explode('-', (isset($_GET['t']) ? $_GET['t'] : date($dformat)));  $t[count($t) - 1]++; echo implode('-', $t); ?>"><img src="/res/img/arrow_forward.svg"></a>
            <a href="?m=d"<?php echo ($_GET['m'] == 'd' ? ' style="background-color:var(--theme-main-color);color:white"': '') ?>>Day</a>
            <a href="?m=m"<?php echo ($_GET['m'] == 'm' ? ' style="background-color:var(--theme-main-color);color:white"': '') ?>>Month</a>
            <a href="?m=y"<?php echo ($_GET['m'] == 'y' ? ' style="background-color:var(--theme-main-color);color:white"': '') ?>>Year</a>
        </div>
                ...
                <?php
                    switch($_GET['m']) {
                        case 'm':
                            ...
                            $entries = $cmanager->getEntries($firstday, $lastday);
                        ...
                        case 'd':
                            ...
                            $entries = $cmanager->getEntries($today, $today.' 23:59:00');
                        ...
                        case 'y':
                            ...
                            $entries = $cmanager->getEntries($year.'-01-01 00:00', $year.'-12-31 00:00');
                            ...
                            for($i = 0; $i < 3; $i++) {
                                ...
                                for($c = 0; $c < 4; $c++) {
                                    ...
                                    if(count($events) > 4) echo '<a href="?m=m&t=', $year, '-', $month, '">', count($events), ' events... (see more)</a>';
