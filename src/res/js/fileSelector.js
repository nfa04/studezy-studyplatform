...
class FileRepresentation {
    ...
    constructor(fileName, fileType, lastEdit) {
        this.name = fileName;
        this.type = fileType;
...
class FileSelector {
    ...
    constructor(htmlNode, fileTypes = null)  {

        htmlNode.className = "studezy_selector";

        this.parentNode = htmlNode;
        this.fileTypes = fileTypes;

        var selectContainer = document.createElement("div");
        selectContainer.className = "studezy_selector_btns";
        this.assetBtn = document.createElement("input");
        this.assetBtn.type = "button";
        this.assetBtn.value = "From assets";
        this.localBtn = document.createElement("input");
        this.localBtn.type = "button";
        this.localBtn.value = "From my computer";
        selectContainer.appendChild(this.assetBtn);
        selectContainer.appendChild(this.localBtn);

        this.parentNode.appendChild(selectContainer);

        this.selectorNode = document.createElement("div");
        this.parentNode.appendChild(this.selectorNode);
    ...
    chooseFromAssets() {
        ...
        return new Promise((resolve, reject) => {
            var assetAPI = new APIRequest("get_assets");
            assetAPI.sendAndFetch({
                fileTypes: JSON.stringify(this.fileTypes)
            }).then((aRes) => {
                var data = JSON.parse(aRes);
                ...
                data.forEach(element => {
                    ...
                    assetLink.addEventListener("click", () => {
                        var confirmScreen = document.createElement("div");
                        var message = document.createElement("div");
                        message.innerText = "Do you want to send this file?";
                        confirmScreen.appendChild(message);
                        var fileIndicator = document.createElement("div");
                        fileIndicator.innerText = element['name'];
                        confirmScreen.appendChild(fileIndicator);
                        var confirmBtn = document.createElement("input");
                        confirmBtn.type = "button";
                        confirmBtn.value = "Confirm";
                        confirmBtn.addEventListener("click", () => {
                            resolve(new FileRepresentation(element['id'], element['type'], element['last_edited']));
                        });
                        confirmScreen.appendChild(confirmBtn);
                        this.selectorNode.replaceChildren();
                        this.selectorNode.appendChild(confirmScreen);
                    });
                    assetLink.href = "javascript:;";
                    assetLink.innerText = element['name'] + "." + mime2ext(element['type']);
                    assetName.appendChild(assetLink);
                    assetRow.appendChild(assetName);
                    var type = document.createElement("td");
                    type.innerText = element['type'];
                    assetRow.appendChild(type);
                    var last_edit = document.createElement("td");
                    last_edit.innerText = element['last_edited'];
                    assetRow.appendChild(last_edit);
                    assetsTable.appendChild(assetRow);
                });
                this.selectorNode.appendChild(assetsTable);
            });
        })
    }
    ...
    chooseFromLocalFS() {
        this.selectorNode.replaceChildren();
        this.progessPercent = document.createElement("span");
        this.progressLabel = document.createElement("span");
        return new Promise((resolve, reject) => {
            var message = document.createElement("div");
            message.innerText = "Please select a file.";
            this.selectorNode.appendChild(message);
            var fileInput = document.createElement("input");
            fileInput.type = "file";
            if(this.fileTypes !== null) fileInput.accept = this.fileTypes.join(", ");
            this.selectorNode.appendChild(fileInput);
            var progressDiv = document.createElement("div");
            progressDiv.appendChild(this.progressLabel);
            progressDiv.appendChild(this.progessPercent);
            this.selectorNode.appendChild(progressDiv);
            var confirmBtn = document.createElement("input");
            confirmBtn.type = "button";
            confirmBtn.value = "Confirm";
            confirmBtn.addEventListener("click", () => {
            ...
            this.selectorNode.appendChild(confirmBtn);
        });
    }
}